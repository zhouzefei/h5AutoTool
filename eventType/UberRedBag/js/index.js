...
// var redbagCode = 'bd5683bd7b8f429984e726059b72d55c';
...
function getTelCode(tel) {
    ...
    $('.getCode').on('click', function() {
        ...
        if ($(this).hasClass('cur')) {
            ...
            getJsonData(
                ajaxUrl + '/redbag/sendAuthCode4NewUser?format=json&phone=' + tel, {},
                function(data) {
                    if (data.code == 0) {
                        $('.mask,.winBox').show();
                        _this.addClass('cur')
                        ...
                        // alert("该红包是为新用户准备的哦");
                        $('.dialogbox .font').text('该红包是为新用户准备的哦');
                $('.dialogbox').show();
                    } else {
                        // alert('验证码发送失败');
...
function getredBagMesg(tel, code) {
    getJsonData(
        ajaxUrl + '/redbag/rush4NewUser?format=json',
         {
